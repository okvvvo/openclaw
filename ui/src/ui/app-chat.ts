...
export function resolveChatSessionsActiveMinutes(settings: UiSettings): number {
  ...
  if (!Number.isFinite(value) || value < 0) {
    return DEFAULT_CHAT_SESSIONS_ACTIVE_MINUTES;
  }
...
function buildNewChatSessionKey(host: ChatHost): string {
  ...
  return `agent:${agentId}:web:dm:new:${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

export async function handleCreateNewSession(host: ChatHost) {
  if (!host.connected || !host.client) {
    return;
  }
  const nextKey = buildNewChatSessionKey(host);
  host.sessionKey = nextKey;
  host.chatMessage = "";
  host.chatAttachments = [];
  host.chatMessages = [];
  ...
  if (host.sessionsResult) {
    const exists = host.sessionsResult.sessions.some((entry) => entry.key === nextKey);
    if (!exists) {
      host.sessionsResult = {
        ...host.sessionsResult,
        count: host.sessionsResult.count + 1,
        sessions: [optimisticEntry, ...host.sessionsResult.sessions],
      };
    }
  }
  ...
  await Promise.all([
    loadSessions(host as unknown as OpenClawApp, {
      activeMinutes: resolveChatSessionsActiveMinutes(host.settings),
    }),
