...
export function renderApp(state: AppViewState) {
  ...
  const chatWindowMinutesRaw = state.settings.chatSessionsActiveMinutes;
  ...
  const chatWindowLabel =
    chatWindowMinutes === 0
      ...
      : chatWindowMinutes < 60
        ? `${chatWindowMinutes}m`
        : `${Math.round((chatWindowMinutes / 60) * 10) / 10}h`;
  const navChatSessions = [...(state.sessionsResult?.sessions ?? [])]
    .toSorted((a, b) => {
    ...
    .slice(0, 20);
  const labelForSession = (key: string, fallback?: string | null) =>
    state.settings.sessionLabels?.[key]?.trim() || fallback?.trim() || key;

  return html`
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""} ${state.navMobileOpen ? "shell--nav-mobile-open" : ""}">
      <header class="topbar">
        <div class="topbar-left">
          <button
            ...
            @click=${() => {
              ...
              if (isMobile) {
                state.setNavMobileOpen(!state.navMobileOpen);
                return;
              }
              state.applySettings({
                ...state.settings,
          ...
          <div class="brand">
            ...
            <div class="brand-text">
              <div class="brand-title">Madangsoe</div>
      ...
      ${state.navMobileOpen ? html`<button class="nav-mobile-backdrop" @click=${() => state.setNavMobileOpen(false)} aria-label="Close sidebar"></button>` : nothing}
      <aside class="nav ${state.settings.navCollapsed ? "nav--collapsed" : ""} ${state.navMobileOpen ? "nav--sheet-open" : ""}">
        <div class="nav-tabs" role="tablist" aria-label="Sidebar sections">
          <button
        ...
        <div class="nav-group nav-group--chat-sessions">
          ...
          <div class="nav-group__items nav-chat-list">
            ${
              navChatSessions.length === 0
                ...
                : navChatSessions.map(
                    (entry) => html`
                      <div
                        class="nav-item nav-chat-item ${entry.key === state.sessionKey ? "active" : ""}"
                        role="button"
                        tabindex="0"
                        @click=${(e: MouseEvent) => {
                          const target = e.target as HTMLElement | null;
                          if (target?.closest(".nav-chat-menu")) {
                            return;
                          }
                        ...
                        title=${entry.key}
                      >
                        <span class="nav-item__text nav-chat-item__text">${labelForSession(entry.key, entry.label ?? entry.displayName)}</span>
                        <details class="nav-chat-menu ${pinnedKeys.has(entry.key) ? "is-pinned" : ""}" @click=${(e: Event) => e.stopPropagation()}>
                          <summary class="nav-chat-menu__trigger" title="Chat actions">
                            <span class="nav-chat-menu__pinmark">${icons.pin}</span>
                            <span class="nav-chat-menu__more">${icons.moreHorizontal}</span>
                          </summary>
                          <div class="nav-chat-menu__popup">
                            <button
                              type="button"
                              class="nav-chat-menu__action"
                              @click=${async (e: Event) => {
                                e.preventDefault();
                                e.stopPropagation();
                                const menu = (e.currentTarget as HTMLElement | null)?.closest(
                                  "details",
                            ...
                            >
                              <span class="nav-chat-menu__icon">${icons.penLine}</span> 이름 바꾸기
                            </button>
                            <button
                              type="button"
                              class="nav-chat-menu__action"
                              @click=${(e: Event) => {
                                e.preventDefault();
                                e.stopPropagation();
                                state.toggleSessionPin(entry.key);
                                const menu = (e.currentTarget as HTMLElement | null)?.closest(
                                  "details",
                                ) as HTMLDetailsElement | null;
                                if (menu) {
                            ...
                            >
                              <span class="nav-chat-menu__icon">${icons.pin}</span>
                              ${pinnedKeys.has(entry.key) ? "고정 해제" : "고정"}
                            </button>
                            <button
                              type="button"
                              class="nav-chat-menu__action danger"
                              @click=${async (e: Event) => {
                                e.preventDefault();
                                e.stopPropagation();
                                const menu = (e.currentTarget as HTMLElement | null)?.closest(
                                ...
                                const deleted = await deleteSessionAndRefresh(state, entry.key);
                                if (!deleted) {
                                  if (menu) {
                                    menu.open = false;
                                  }
                                ...
                                const nextPins = (state.settings.pinnedSessionKeys ?? []).filter(
                                  (key) => key !== entry.key,
                                );
                                const nextLabels = { ...state.settings.sessionLabels };
                                delete nextLabels[entry.key];
                                if (state.sessionKey === entry.key) {
                                  const fallback =
                                    state.sessionsResult?.sessions?.[0]?.key ?? "main";
                                  state.sessionKey = fallback;
                                  state.applySettings({
                                    ...state.settings,
                                    sessionKey: fallback,
                                    lastActiveSessionKey: fallback,
                                    pinnedSessionKeys: nextPins,
                                    sessionLabels: nextLabels,
                                  });
                                  void loadChatHistory(state);
                                  void refreshChatAvatar(state);
                            ...
                            >
                              <span class="nav-chat-menu__icon">${icons.trash}</span> 삭제
                            </button>
                          </div>
                        </details>
                      </div>
        ...
        ${
          state.navSidebarTab === "menu"
            ? html`
        <div class="nav-bottom-groups">
          ${TAB_GROUPS.map((group) => {
            const tabs = group.tabs.filter((tab) => tab !== "chat");
            if (tabs.length === 0) {
              return nothing;
            }
        ...
        ${
          state.tab === "chat"
            ? renderChat({
                ...
                onNewSession: () => void state.handleCreateNewSession(),
