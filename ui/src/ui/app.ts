...
import {
  ...
  handleCreateNewSession as handleCreateNewSessionInternal,
...
export class OpenClawApp extends LitElement {
  ...
  @state() navSidebarTab: "chats" | "menu" = "chats";
  ...
  async handleCreateNewSession() {
    await handleCreateNewSessionInternal(
      this as unknown as Parameters<typeof handleCreateNewSessionInternal>[0],
    );
  }

  toggleSessionPin(sessionKey: string) {
    const key = sessionKey.trim();
    if (!key) {
      return;
    }
    const pins = this.settings.pinnedSessionKeys ?? [];
    const nextPins = pins.includes(key) ? pins.filter((entry) => entry !== key) : [...pins, key];
    this.applySettings({ ...this.settings, pinnedSessionKeys: nextPins });
  }
  ...
  setNavSidebarTab(next: "chats" | "menu") {
    this.navSidebarTab = next;
  }

  setNavMobileOpen(next: boolean) {
    this.navMobileOpen = next;
  }
